...
- Used by: `Madbox.Animation` (`EntityAttributeAnimatorDriver` bridges attributes to `AnimationController`), `Madbox.Player` (`Player` / `PlayerAttribute`), `Madbox.Enemies` (`Enemy` / `EnemyAttribute`), `Madbox.GameView` (`ProjectileData` and thin subclasses of generic runners/drivers).
...
| `Entity` | `attributeEntries`, `attributeModifiers`, `Add/Remove/ClearAttributeModifier`, `Get/SetFloat/BoolAttribute` (set updates base), `AttributeValueChanged` (effective value). Recomputes on modifier changes, base changes, and after deserialize. Subclass for typed accessors. |
...
- Assembly: `Madbox.Entities.Tests` (EditMode) for base `Entity` attribute behavior.
