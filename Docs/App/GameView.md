...
- Owns player **view** behaviors that are GameView-specific (movement/attack), and generic `AnimationController` (cross-fade by state name, bool/float parameters). Attribute → animator wiring uses **`EntityAttributeAnimatorDriver`** from **`Madbox.Animation`**. Player **entity** types (`Player`, `PlayerAttribute`) and shared player orchestration types (`PlayerBehaviorRunner`, `IPlayerBehavior`, `PlayerInputContext`, `WeaponVisualController`, `PlayerWeaponController`) live in **`Madbox.Player`**; enemy **entity** types (`Enemy`, `EnemyAttribute`) live in **`Madbox.Enemies`**.
...
- References **`Madbox.Entities`** only for shared `Entity` patterns on view-layer components (e.g. `ProjectileData`); does not own core simulation.
...
| `EntityAttributeAnimatorDriver` | Maps `EntityAttribute` → `AnimationAttribute` | `Entity.AttributeValueChanged` | `AnimationController` parameters | No-op if link or controller missing |
| `Projectile` | Optional forward motion, `ScheduleDestroyAfterSeconds`, trigger impact self-destruct; applies `ProjectileData.Damage` to `Damageable` on the other collider (`GetComponentInParent`), fallback `damageWhenNoProjectileData` | Start/Update/trigger | Moves along forward when enabled | Trigger collider + Rigidbody; targets expose `Damageable` + `Entity`; use **Projectile** layer |
...
| `Arena` | Level-scene marker for bounds and spawns | Inspector: optional box collider, spawn transforms, optional **Battle Virtual Camera** (`CinemachineVirtualCamera`) | `TryGetWorldBounds`, spawn world positions, `BattleVirtualCamera`, `TryFindInScene` | Bounds false when no box collider; spawns fall back to arena transform; battle camera follow/look-at are assigned when spawning a session player (`BattleGameFactory`) |

## Setup / Integration

1. Add `Madbox.GameView` reference to consuming assemblies if needed (prefabs only do not require code references).
2. **Arena (level scenes)**: Add `Arena` to a root or child in each Addressable level scene. Optionally assign a `BoxCollider` on the same object (or drag one into **Area Bounds Source**) for `TryGetWorldBounds`. Set **Enemy Spawn Point** / **Player Spawn Point** for explicit spawn transforms; otherwise the arena transform position is used. Optionally assign **Battle Virtual Camera** to the level’s Cinemachine virtual camera so `BattleGameFactory` can set both **Follow** and **Look At** on the spawned player. After `SceneManager` / Addressables loads the level scene, call `Arena.TryFindInScene(loadedScene, out var arena)` to read positions and bounds.
3. **Weapon loadout**: Create a **`PlayerLoadoutDefinition`** (see **`Docs/Meta/Levels.md`**, menu **Create > Madbox > Player > PlayerLoadout**). Assign Addressables for the player prefab (must include `WeaponVisualController` with the same number of sockets as weapon entries) and a list of weapon prefabs. Register the definition asset under the address **`Player Loadout`** (see `PlayerLoadoutAssetProvider`) so bootstrap preload registers it and **`PlayerService`** receives it in its constructor. Call `PlayerFactory.CreateReadyPlayerAsync` from **`Madbox.App.Bootstrap.Player`** (scoped in bootstrap). Switch visible weapon with `WeaponVisualController.SetSelectedWeaponIndex` (weapons stay instantiated; inactive slots are disabled).
...
7. For attack speed scaling: add float parameter `AttackSpeedMultiplier` (default 1) on the Animator Controller; enable **Speed Parameter** on attack states only, set to `AttackSpeedMultiplier`. Create an `AnimationAttribute` asset with that parameter name. On the player, add a `PlayerAttribute` for attack speed, list it on `Player`, and add `EntityAttributeAnimatorDriver` with a link from that attribute to the `AttackSpeedMultiplier` `AnimationAttribute`. For non-player animators, set the float on `AnimationController` manually or via a small view script.
...
4. **Tune attack speed**: Change `Player` attribute entries (e.g. attack speed stat) on the hero; idle/run states should keep speed parameter inactive so only attack accelerates.
...
- Moved safe shared player orchestration/weapon types (`PlayerBehaviorRunner`, `IPlayerBehavior`, `PlayerInputContext`, `WeaponVisualController`, `PlayerWeaponController`) to `Madbox.Player`; kept GameView-specific player view behaviors (`PlayerMovementViewBehavior`, `PlayerAttackViewBehavior`) in `Madbox.GameView`. Attribute → animator uses `EntityAttributeAnimatorDriver` in `Madbox.Animation`.
- Documented `WeaponVisualController` and pointed loadout authoring (`PlayerLoadoutDefinition` in `Madbox.Levels`) + `PlayerService` / `PlayerFactory` in bootstrap to keep `Madbox.GameView` free of Addressables references.
- Initial module doc for animation event routing, player view behaviors, and attack speed multiplier.
