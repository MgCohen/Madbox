...
| `EntityAttributeEntry` | Base serialized list item on **`Entity`**: attribute reference, float value, optional `UnityEvent<float>` when the value changes. |
| `Player` | Extends **`Entity`**: references `isAliveAttribute` and `canMoveAttribute`, inherits `attributeEntries`, exposes `IsAlive` / `CanMove` (via attributes) plus inherited attribute getters/setters and `AttributeValueChanged`. |
| `PlayerAttributeAnimatorDriver` | Thin subclass of **`EntityAttributeAnimatorDriver{Player}`**; maps each `PlayerAttribute` to an `AnimationAttribute` on `AnimationController`. |
...
2. Assign `IsAlive` and `CanMove` attribute assets on `Player`, and add one list entry per attribute (including those two) with the default float (`1` for bool true).
