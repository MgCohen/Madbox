...
| `EntityAttributeAnimatorDriver<TData>` | Maps `EntityAttribute` → `AnimationAttribute` on `AnimationController` when attribute values change (generic over `Entity` subclasses). |
