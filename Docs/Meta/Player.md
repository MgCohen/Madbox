...
- Purpose: Player **entity** MonoBehaviour and ScriptableObject attribute types (`PlayerData`, `PlayerAttribute`) shared by bootstrap spawn, Game View behaviors, and prefabs.
...
- Owns `PlayerData` (`EntityData` subclass with `IsAlive` / `CanMove` when attributes are assigned).
...
| Symbol | Role |
|--------|------|
| `PlayerData` | Serialized attribute entries; `IsAlive` / `CanMove` via assigned `PlayerAttribute` references. |
