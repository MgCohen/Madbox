...
4. UnityEngine use is per-assembly: some modules under `Core/` (for example `Madbox.Entities`) intentionally reference `UnityEngine`; others may use `noEngineReferences: true`. Do not assume “Core” means engine-free—check the `.asmdef`.
...
2. `InstallCore`
   ...
   - Register shared state and policies (plain C# where modules are agnostic; engine-facing Core modules as needed).
3. `InstallMeta`
...
3. Cache + apply live ops in policy-defined timing (for example: apply remote tuning on the next session when that is the chosen policy).
...
3. Optional: per-assembly policy checks (for example assemblies that declare `noEngineReferences: true` stay free of `UnityEngine`); do not treat all of Core as Unity-free.
