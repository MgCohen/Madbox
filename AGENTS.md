...
Madbox is a **sample** modular Unity project (C#) with architecture boundaries enforced through documentation, explicit assembly/project dependencies, and custom Roslyn analyzers. The codebase has **limited scope**: it demonstrates how chosen technologies and patterns work together (MVVM-style UI, VContainer, Addressables, LiveOps/Cloud Code, analyzers, tests). It is **not** positioned as a universal “best stack” for every production game—use [`Architecture.md`](Architecture.md) and [`Docs/`](Docs/) as the authority on what is actually implemented.
...
2. **Separation of Concerns**: Keep App/UI and scene-specific presentation separate from unrelated modules. The `Core/` folder is not “Unity-free by definition”—assemblies such as **`Madbox.Entities`** intentionally use `UnityEngine` (for example `MonoBehaviour` components for health and lifecycle). Do not treat “under Core” as a ban on engine APIs.
3. **Modular Integrity**: Declare all dependencies explicitly via `.asmdef`/`.csproj`. Never bypass architectural boundaries.
4. **MonoBehaviours**: Use them in bootstrap, App/Infra presentation, and in Core modules that own engine-facing gameplay building blocks (for example **`Madbox.Entities`**). Do not add MonoBehaviours to assemblies that are intentionally Unity-free (`noEngineReferences: true` in their `.asmdef`).
