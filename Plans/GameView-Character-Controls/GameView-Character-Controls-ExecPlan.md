...
- [x] Authored initial ExecPlan with milestone order aligned to requested scope: basic controls first, visual behavior mapping next, intent split last.
- [x] Execute Milestone 1: Implemented `VirtualJoystickInput` + `PlayerMovementViewBehavior` and `MainScene` auto-wiring so drag direction drives hero movement and walk/idle animation switching.
- [x] Execute Milestone 2: Implemented `PlayerAttackAnimationBehavior` (animation-only trigger path with debug key) plus tests validating attack lock trigger behavior.
...
Revision Note (Codex): Created initial ExecPlan for phased visual character controls integration: joystick movement and walk animation first, animation-only attack second, behavior mapping third, and input-intent separation last.
Revision Note (Codex): Marked Milestones 1 and 2 complete after implementing view-side joystick movement, walk/idle animation switching, animation-only attack behavior, and focused EditMode/PlayMode validation.
