...
Reference implementation for enemies: **`EnemyFactory`** in `Assets/Scripts/Core/Enemies/Runtime/EnemyFactory.cs` only instantiates; **`BattleGameFactory`** in `Assets/Scripts/Core/Battle/Runtime/BattleGameFactory.cs` loads Addressables then spawns. For the player, **`PlayerFactory`** may combine load and spawn in one type or split internally, but the **observable contract** to the game is: **input loadout (from `PlayerService` or passed in), output one ready player**. **Socket** means a **`Transform`** serialized on a player behaviour (for example **`WeaponVisualController`**) where weapon instances are parented.
