...
| **Cloud Code** | Call module endpoints after UGS is ready. | `Assets/Scripts/Infra/LiveOps/Runtime/CloudCode/` (`Madbox.LiveOps.CloudCode`) | `IUgsInitializationService` (from `Madbox.Ugs`), Cloud Code SDK |
...
**Assembly layout:** `Madbox.Ugs` is its own assembly (`Assets/Scripts/Infra/Ugs/Runtime/Madbox.Ugs.asmdef`). `Madbox.LiveOps` holds LiveOps runtime namespaces (`CloudCode`, `Implementation`, etc.) and references `Madbox.Ugs` for `IUgsInitializationService` / `UgsInitializationService`. A separate `Madbox.CloudCode` assembly also exists under `Infra/CloudCode` for non–LiveOps consumers; LiveOps registers the LiveOps-namespaced `CloudCodeModuleService` in `LiveOpsInstaller`.
...
- [x] (2026-03-21) Refactor: Separated **Cloud Code** and **LiveOps** into distinct namespaces under `Infra/LiveOps/Runtime/` (UGS later consolidated into `Infra/Ugs` as **`Madbox.Ugs`**); LiveOps depends on Cloud Code only; DTOs supplied via **`Madbox.LiveOps.DTO`** DLL + `csc.rsp`; updated this plan to match.
...
- `Assets/Scripts/Infra/LiveOps/Runtime/CloudCode/*`, `Implementation/*`
