...
|--------|------|----------------|------------|
| **UGS** | Initialize Unity Services and anonymous sign-in when something needs Cloud Code. | `Assets/Scripts/Infra/LiveOps/Runtime/Ugs/` (`Madbox.LiveOps.Ugs`) | Unity Services packages only |
| **Cloud Code** | Call module endpoints after UGS is ready. | `Assets/Scripts/Infra/LiveOps/Runtime/CloudCode/` (`Madbox.LiveOps.CloudCode`) | `IUgsInitializationService`, Cloud Code SDK |
| **LiveOps** | Map app calls (ping, future ops) to Cloud Code payloads and DTOs; no direct UGS usage. | `Assets/Scripts/Infra/LiveOps/Runtime/Implementation/` (`Madbox.LiveOps`) | `ICloudCodeModuleService` only (not UGS directly) |

**Registration order** (see `LiveOpsInstaller`): UGS → Cloud Code → LiveOps.

**Assembly layout:** One Unity assembly definition, `Madbox.LiveOps`, holds all three areas so batch-mode compilation produces a single `Madbox.LiveOps` project; logical separation is by folder and namespace. Separate asmdefs for UGS/Cloud Code were deferred where Bee batch builds did not emit per-assembly references reliably.
...
- [x] (2026-03-21) Refactor: Separated **UGS**, **Cloud Code**, and **LiveOps** into distinct namespaces under `Infra/LiveOps/Runtime/`; LiveOps depends on Cloud Code only; DTOs supplied via **`Madbox.LiveOps.DTO`** DLL + `csc.rsp`; updated this plan to match.
...
Unity exposes a scoped LiveOps surface: **`ILiveOpsService`** + **`LiveOpsService`**, with **`ICloudCodeModuleService`** / **`CloudCodeModuleService`** and **`IUgsInitializationService`** / **`UgsInitializationService`** registered in **`LiveOpsInstaller`**. Fallback mode remains the default for resilient local loads.
...
Milestone 1 starts with Unity bridge creation first. Create `ILiveOpsService` and `LiveOpsService` in `Assets/Scripts/Infra/LiveOps/Runtime/` plus installer registration in `Assets/Scripts/Infra/LiveOps/Container/`. Add a local fallback or stub execution mode so application startup and dependency injection load path can succeed before Cloud Code exists.
...
- `Assets/Scripts/Infra/LiveOps/Runtime/Ugs/*`, `CloudCode/*`, `Implementation/*`
- `Assets/Scripts/Infra/LiveOps/Container/*` installer/wiring
...
- Unity:
  ...
  - `IUgsInitializationService` / `UgsInitializationService`
