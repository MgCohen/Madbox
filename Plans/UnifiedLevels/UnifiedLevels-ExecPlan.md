...
- [x] **`LevelDefinition.LevelId`** added; must match remote-config ordered IDs.
...
- [x] **`CompleteLevelResponse(bool succeeded)`**; Cloud Code returns **`Succeeded`** only (no embedded **`LevelGameData`**).
- [x] **`LevelService.CompleteLevelAsync`** on client; **`IAvailableLevelsService`** / **`AvailableLevelsService`** registered in **`BootstrapMetaInstaller`** with default empty **`IReadOnlyList<LevelDefinition>`**.
- [ ] (deferred) Wire **MainMenu** to **`IAvailableLevelsService`** (optional follow-up).
- [x] **`Docs/Meta/LiveOpsLevel.md`**, **`Docs/Meta/Levels.md`** updated.
...
**9. Documentation.** Update `Docs/Meta/LiveOpsLevel.md`, `Docs/Meta/Levels.md`, and this plan’s **Progress** when behavior is final.
...
- Initial ExecPlan authored from repository inspection and user specification; progression rule and unified client contract recorded in **Decision Log**.

- Added **Explicit type sketches** (minimal config, persistence, complete-level DTOs, `LevelService.CompleteLevelAsync`); **Decision Log** entries for bool-only **`CompleteLevelResponse`** and request-by-id only; clarified deferred wiring for post-complete data refresh.

- (Superseded) Earlier draft added **repo-wide deletion** of **`MultiProgressModuleData.cs`** and **Tutorial** migration in the same plan.

- **Scoped** `MultiProgressModuleData` to **Level only**: Level DTOs stop using the base; **Tutorial** and **`MultiProgressModuleData.cs`** stay until a **later** refactor; removed former step 10, Progress item, and deletion-focused validation.

- **Implementation pass:** DTO + Cloud Code + client **`LevelService`**, **`IAvailableLevelsService`**, docs, tests; **`validate-changes`** may require closing Unity for compilation precheck.
