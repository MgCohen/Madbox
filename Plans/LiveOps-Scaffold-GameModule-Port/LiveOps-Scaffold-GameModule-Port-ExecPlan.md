...
- [x] Authored ExecPlan (initial): scope, source layout, Unity integration expectations, validation loop.
- [x] Copied Scaffold `GameModuleDTO`, `Project`, and `Utility` into `LiveOps/LiveOps.DTO`, `LiveOps/Project`, `LiveOps/Utility` (sources unmodified).
- [x] Added `LiveOps/LiveOps.sln`, `LiveOps.DTO/Madbox.LiveOps.DTO.csproj` (AssemblyName `Madbox.LiveOps.DTO`), `Project/LiveOps.csproj` (AssemblyName `LiveOps`); removed duplicate `OutputPath` from DTO csproj to avoid double `netstandard2.1` folder.
- [x] Built `dotnet build LiveOps\LiveOps.sln -c Release`; copied `Utility.dll` and `Madbox.LiveOps.DTO.dll` to `Assets/Plugins/`.
- [x] Removed Unity `Madbox.LiveOps` / Container / Tests folders; stripped bootstrap ping and `LiveOpsInstaller` wiring; removed `Madbox.LiveOps` + DTO precompile from `Madbox.Bootstrap.asmdef`; removed `Assets/Test.cs` (SCA0022).
- [x] Removed `Madbox.LiveOps*.csproj` from `Madbox.sln`; patched generated `Madbox.Bootstrap.Runtime.csproj` and `Assembly-CSharp.csproj` until Unity regen (see Surprises).
- [x] Updated `Docs/LiveOps.md`; added plugin metas for `Madbox.LiveOps.Utility`.
...
**Shipped :** Backend under `LiveOps/` matches Scaffold GameModule stack (Utility + DTO + main). Unity no longer has a `Madbox.LiveOps` client assembly; ping/pong and `ILiveOpsService` were removed with bootstrap ping lines only—no replacement Cloud Code API. Plugins: `Madbox.LiveOps.DTO.dll` + `Utility.dll` in `Assets/Plugins/`. **`Assets/Scripts/Core/LiveOps/LiveOps.ccmr`** left in place for manual repackage.
...
- Initial ExecPlan authored from user request (verbatim Scaffold port, three libraries, LiveOps naming, plugin rebuild, ping/pong removal allowed).

- Utility **vendored inside** `LiveOps/Utility/`; clarified “single LiveOps” = one folder + one solution with **three** projects, not one merged `.csproj` (framework split DTO vs host).

- Ping/pong: if missing from ported contracts, **minimal deletion** of usages only—no refactor or replacement endpoint in scope.

- **Execution complete** (agent): Scaffold copy, `LiveOps.sln`, plugin deploy, Unity LiveOps assembly removal, `Docs/LiveOps.md`; validation gate blocked by Unity project lock—user must re-run locally.
