...
- [x] Authored initial ExecPlan and executed first migration pass (Addressables integration + tests + clean quality gate).
- [x] Completed first migration pass with clean `.agents/scripts/validate-changes.cmd`.
- [x] Re-scoped plan to cache/residency model based on review feedback: remove navigation preload registration, keep startup-driven config load policy external, add runtime instance cache, keep `INavigation` unchanged.
- [x] Execute Milestone 1: Removed preload registration logic from `NavigationInstaller` and reverted container asmdef dependencies to navigation-only wiring scope.
- [x] Execute Milestone 2: Added `NavigationPrefabStore` resident handle service and warmed all valid navigation asset references at provider startup.
- [x] Execute Milestone 3: Added `NavigationViewInstanceBuffer` and wired non-context point disposal to return instances to buffer (with bounded cache size policy).
- [x] Execute Milestone 4: Updated `NavigationPoint` readiness model and `NavigationTransitions` processing to await target readiness before opening transitions.
- [x] Execute Milestone 5: Expanded tests/docs and passed `.agents/scripts/validate-changes.cmd` with clean gate (`TOTAL:0`).
...
Revision Note (Codex): Created initial ExecPlan for migrating Navigation to Addressables.
Revision Note (Codex): Executed first migration pass with clean quality gate.
Revision Note (Codex): Re-scoped to second-pass refactor for preload decoupling, prefab residency, instance cache, and readiness-aware transitions without API changes.
Revision Note (Codex): Executed second-pass refactor end-to-end, updated docs/tests, and validated clean full quality gate.
