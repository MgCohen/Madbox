...
- [x] Authored initial polish-and-sync ExecPlan from current Addressables runtime state and user requirements.
- [x] Revised ExecPlan to include Asset-layer bootstrap revamp, single preload file flow, and child-scope delegated registration from Addressables async initialization.
...
- Observation: Preload label string is currently duplicated and hardcoded.
  Evidence: `Assets/Scripts/Infra/Addressables/Runtime/Implementation/AddressablesStartupCoordinator.cs`, `Assets/Scripts/Infra/Addressables/Tests/AddressablesGatewayTests.cs`, `Docs/Assets/Addressables.md`.
...
Update `Docs/Assets/Addressables.md` and `Docs/App/Bootstrap.md` with new Asset-layer startup sequence and preload injection semantics.
...
Revision Note (Codex): Created initial polish-focused plan for constants, regression test, and deferred sync handle API.
Revision Note (Codex): Expanded plan with Asset-layer bootstrap revamp, single preload file startup, and child-scope instance delegation during async initialization.
