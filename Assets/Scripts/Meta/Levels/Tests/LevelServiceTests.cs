...
namespace Madbox.Levels.Tests
{
    public sealed class LevelServiceTests
    {
        ...
        public void CompleteLevelAsync_DelegatesToLiveOps()
        {
            ...
            LevelDefinition def = CreateLevelDefinition(3);
            CompleteLevelResponse result = sut.CompleteLevelAsync(def, CancellationToken.None).GetAwaiter().GetResult();
        ...
        public void CompleteLevelAsync_UsesCompletedLevelIdToUpdateLocalStates()
        {
            ...
            LevelDefinition levelOne = CreateLevelDefinition(1);
            LevelDefinition levelTwo = CreateLevelDefinition(2);
            FakeGroupProvider provider = new FakeGroupProvider(new[] { levelOne, levelTwo });
            ...
            using (IObjectResolver container = builder.Build())
            {
                ...
                _ = sut.CompleteLevelAsync(levelOne, CancellationToken.None).GetAwaiter().GetResult();
        ...
        {
            return CreateConfig(ids, rewardPerLevel: 0);
        }

        private static LevelConfig CreateConfig(int[] ids, int rewardPerLevel)
        {
            LevelConfig config = new LevelConfig();
            FieldInfo levelsField = typeof(LevelConfig).GetField("_levels", BindingFlags.NonPublic | BindingFlags.Instance);
            levelsField.SetValue(config, new List<int>(ids));
            FieldInfo rewardField = typeof(LevelConfig).GetField("_rewardPerLevel", BindingFlags.NonPublic | BindingFlags.Instance);
            rewardField.SetValue(config, rewardPerLevel);
            return config;
        }
        ...
        private sealed class DelayedStubLiveOpsForComplete : ILiveOpsService
        {
            private readonly TaskCompletionSource<CompleteLevelResponse> responseTcs = new TaskCompletionSource<CompleteLevelResponse>();
            ...
            public Task<TResponse> CallAsync<TResponse>(ModuleRequest<TResponse> request, CancellationToken cancellationToken = default) where TResponse : ModuleResponse
            {
                return AwaitResponse<TResponse>(cancellationToken);
            }

            private async Task<TResponse> AwaitResponse<TResponse>(CancellationToken cancellationToken) where TResponse : ModuleResponse
            {
                CompleteLevelResponse response = await responseTcs.Task;
                cancellationToken.ThrowIfCancellationRequested();
                return (TResponse)(object)response;
            }

            public void Complete(CompleteLevelResponse response)
            {
