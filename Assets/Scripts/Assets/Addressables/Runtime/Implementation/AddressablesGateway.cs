...
namespace Madbox.Addressables
{
    public sealed class AddressablesGateway : IAddressablesGateway, IAsyncLayerInitializable
    {
        ...
        private static async Task EnsureAddressablesRuntimeInitializedAsync(CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            {
                // InitializeAsync may auto-invalidate the handle when the operation completes; Release throws if the handle is stale.
                if (handle.IsValid())
                {
