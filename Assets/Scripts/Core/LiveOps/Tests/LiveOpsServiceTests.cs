...
namespace Madbox.LiveOps.Tests
{
    public sealed class LiveOpsServiceTests
    {
        ...
        [Test]
        public void CallAsync_CompleteLevelResponse_WithNestedGoldChangedResponse_DispatchesGoldChangedHandler()
        {
            GoldChangedResponse nested = new GoldChangedResponse(100, 25);
            CompleteLevelResponse root = new CompleteLevelResponse(true, null, 1);
            root.Responses.Add(nested);
            TypedResponseCloudStub<CompleteLevelResponse> cloudCode = new TypedResponseCloudStub<CompleteLevelResponse>(root);
            CountingGoldChangedHandler handler = new CountingGoldChangedHandler();
            ContainerBuilder builder = new ContainerBuilder();
            builder.RegisterInstance(cloudCode).As<ICloudCodeModuleService>();
            builder.RegisterInstance(handler).AsImplementedInterfaces();
            builder.Register<LiveOpsService>(Lifetime.Singleton);
            ...
            {
                LiveOpsService sut = container.Resolve<LiveOpsService>();
                CompleteLevelRequest request = new CompleteLevelRequest(1);
                sut.CallAsync(request).GetAwaiter().GetResult();
            }

            Assert.That(handler.InvocationCount, Is.EqualTo(1));
            Assert.That(handler.LastDiff, Is.EqualTo(25));
        }
        ...
        private sealed class TypedResponseCloudStub<TExpected> : ICloudCodeModuleService where TExpected : ModuleResponse
        {
            private readonly TExpected response;

            public TypedResponseCloudStub(TExpected response)
            {
                this.response = response;
            }
        ...
        private sealed class CountingGoldChangedHandler : IResponseHandler<GoldChangedResponse>
        {
            public int InvocationCount { get; private set; }

            public long LastDiff { get; private set; }

            public void Handle(GoldChangedResponse response)
            {
                InvocationCount++;
                LastDiff = response.Diff;
            }
        }
