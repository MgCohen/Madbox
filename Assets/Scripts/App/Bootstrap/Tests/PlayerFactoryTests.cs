...
namespace Madbox.App.Bootstrap.Tests
{
    public sealed class PlayerFactoryTests
    {
        ...
        public void CreateReadyPlayerAsync_ReturnsPlayerWithWeaponsWired()
        {
            ...
            GameObject playerPrefabRoot = BuildPlayerPrefabWithWeaponSocket(moveSpeed, 10f);
            ...
            Task<Player> task = factory.CreateReadyPlayerAsync(null, Vector3.zero, Quaternion.identity, sessionAddressableHandles);
            ...
            {
                if (!sessionAddressableHandles[i].IsReleased)
                {
                    sessionAddressableHandles[i].Release();
                }
            }
        ...
        [Test]
        public void CreateReadyPlayerAsync_ThrowsWhenWeaponVisualControllerMissing()
        {
            EntityAttribute moveSpeed = ScriptableObject.CreateInstance<EntityAttribute>();
            moveSpeed.name = "MoveSpeed";
            GameObject playerPrefabRoot = BuildPlayerPrefabWithoutWeaponVisual(moveSpeed, 10f);
            GameObject weaponPrefab0 = new GameObject("weaponPrefab0");

            PlayerLoadoutDefinition loadout = ScriptableObject.CreateInstance<PlayerLoadoutDefinition>();
            SetPrivateAssetReference(loadout, "playerPrefab", new AssetReference("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"));
            SetPrivateWeaponPrefabs(loadout, new AssetReference("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"));
        ...
        private static GameObject BuildPlayerPrefabWithoutWeaponVisual(EntityAttribute baseAttribute, float baseValue)
        {
        ...
        private static GameObject BuildPlayerPrefabWithWeaponSocket(EntityAttribute baseAttribute, float baseValue)
        {
            GameObject root = new GameObject("playerPrefab");
            Player player = root.AddComponent<Player>();
            SetBaseAttributeEntry(player, baseAttribute, baseValue);
            ...
            typeof(WeaponVisualController).GetField("weaponSocket", BindingFlags.NonPublic | BindingFlags.Instance)
                ?.SetValue(visual, socketGo.transform);
            return root;
        }
