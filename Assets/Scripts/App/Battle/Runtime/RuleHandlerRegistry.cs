...
namespace Madbox.Battle
{
    public sealed class RuleHandlerRegistry
    {
        ...
        public RuleHandlerRegistry()
        {
            RegisterBuiltinMappings();
        }

        private void RegisterBuiltinMappings()
        {
            Register<TimeElapsedCompleteRule, TimeElapsedCompleteRuleHandler>();
            Register<KillAllEnemiesCompleteRule, KillAllEnemiesCompleteRuleHandler>();
            Register<PlayerDeathLoseRule, PlayerDeathLoseRuleHandler>();
        }

        private void Register<TRule, THandler>() where TRule : LevelRuleDefinition where THandler : RuleHandler<TRule>
        {
