...
namespace Scaffold.MVVM.Tests
{
    public class MVVMTests
    {
        ...
        [Test]
        public void TreeBinding_RegisterBindCollection_WhenSourceCollectionReplaced_ClearsOldTargetsBeforeSeedingNewOnes()
        {
            RebindableCollectionBindingState source = new RebindableCollectionBindingState();
            source.Items.Add(1);
            source.Items.Add(2);
            TrackingCollectionHandler handler = new TrackingCollectionHandler();
            TreeBinding bindings = new TreeBinding();
            ...
            Assert.AreEqual(2, handler.AddCalls);
            CollectionAssert.AreEquivalent(new[] { "1", "2" }, handler.Values);

            source.Items = new ObservableCollection<int> { 9, 10 };
            string key = ExpressionsUtility.GetPropertyName(() => source.Items);
            bindings.UpdateBind(key);

            Assert.AreEqual(4, handler.AddCalls);
            Assert.AreEqual(2, handler.RemoveCalls);
            CollectionAssert.AreEquivalent(new[] { "9", "10" }, handler.Values);
        }
        ...
        public void TreeBinding_DisposeCollectionBinding_DetachesAndStopsFutureCollectionUpdates()
        {
            ...
            Assert.AreEqual(2, handler.AddCalls);
            Assert.AreEqual(2, handler.RemoveCalls);
            CollectionAssert.IsEmpty(handler.Values);
            Assert.DoesNotThrow(() => bindings.UpdateBind(key));
        }
    ...
    public class RebindableCollectionBindingState
    {
