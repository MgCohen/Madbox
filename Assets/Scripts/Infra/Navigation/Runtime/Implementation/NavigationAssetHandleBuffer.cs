...
namespace Scaffold.Navigation
{
    internal sealed class NavigationAssetHandleBuffer
    {
        ...
        public bool TryTake(ViewConfig config, out IAssetHandle<GameObject> handle)
        {
            ...
            if (!byConfig.TryGetValue(config, out Stack<IAssetHandle<GameObject>> pool)) return false;
            while (pool.Count > 0)
            {
                IAssetHandle<GameObject> cached = pool.Pop();
                if (!IsReusable(cached)) continue;
                handle = cached;
                return true;
            }
            return false;
        ...
        {
            if (config == null || !IsReusable(handle)) return false;
            if (!byConfig.TryGetValue(config, out Stack<IAssetHandle<GameObject>> pool)) pool = RegisterPool(config);
            if (pool.Count >= maxPerView) return false;
            pool.Push(handle);
            return true;
        }

        private static bool IsReusable(IAssetHandle<GameObject> handle)
        {
            if (handle == null || handle.IsReleased) return false;
