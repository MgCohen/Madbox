...
namespace Scaffold.Navigation
{
    internal sealed class AddressablesNavigationPointStrategy : INavigationPointStrategy
    {
        ...
        {
            IAssetHandle<GameObject> handle = TryGetBufferedHandle(config);
            ...
            point = new NavigationPoint(
                ...
                disposed =>
                {
                    ReleaseOrBufferHandle(config, handleSlot);
                    if (disposed == null || disposed.IsSceneView || disposed.View == null) return;
        ...
        private IAssetHandle<GameObject> TryGetBufferedHandle(ViewConfig config)
        {
            return assetHandleBuffer.TryTake(config, out IAssetHandle<GameObject> cached) ? cached : null;
        }
        ...
        private void ReleaseOrBufferHandle(ViewConfig config, IAssetHandle<GameObject>[] handleSlot)
        {
            IAssetHandle<GameObject> handle = handleSlot[0];
            if (handle == null) return;
            handleSlot[0] = null;
            if (assetHandleBuffer.Return(config, handle)) return;
            handle.Release();
        }

        private static void DestroyViewObject(UnityEngine.Object instance)
        {
            if (instance == null)
            {
                return;
