...
namespace Madbox.SceneFlow.Tests
{
    public sealed class SceneFlowServiceTests
    {
        ...
        [Test]
        public void LoadAdditiveAsync_WhenManageShell_NullShell_DoesNotThrow()
        {
            RunAsync(async () =>
            {
                FakeAddressablesSceneOperations operations = new FakeAddressablesSceneOperations();
                SceneFlowService service = new SceneFlowService(operations, null);
                AssetReference reference = new AssetReference("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");
                ...
                await service.UnloadAsync(result);
            });
        }
        ...
        [Test]
        public void UnloadAsync_WhenSceneLoadHandleInvalidBeforeUnload_CompletesWithoutCallingUnloadSceneAsync()
        {
            RunAsync(async () =>
            {
                FakeAddressablesSceneOperations operations = new FakeAddressablesSceneOperations();
                SceneFlowService service = new SceneFlowService(operations, new RecordingSceneFlowBootstrapShell());
                AssetReference reference = new AssetReference("11111111111111111111111111111111");

                SceneFlowLoadResult loadResult = await service.LoadAdditiveAsync(reference, new SceneFlowLoadOptions(false));
                Addressables.Release(operations.LastLoadHandle);
                await service.UnloadAsync(loadResult);

                Assert.AreEqual(0, operations.UnloadCallCount);
        ...
        [Test]
        public void UnloadAsync_WhenUnloadHandleInvalidAfterAwait_CompletesWithoutThrowing()
        {
            RunAsync(async () =>
            {
                FakeInvalidUnloadHandleAfterCompleteOperations operations = new FakeInvalidUnloadHandleAfterCompleteOperations();
                SceneFlowService service = new SceneFlowService(operations, new RecordingSceneFlowBootstrapShell());
                AssetReference reference = new AssetReference("ffffffffffffffffffffffffffffffff");

                SceneFlowLoadResult loadResult = await service.LoadAdditiveAsync(reference, new SceneFlowLoadOptions(false));
                await service.UnloadAsync(loadResult);

                Assert.AreEqual(1, operations.UnloadCallCount);
            });
        }
        ...
        /// <summary>
        /// Simulates Addressables invalidating the unload handle after completion (reading Status would throw).
        /// </summary>
        private sealed class FakeInvalidUnloadHandleAfterCompleteOperations : IAddressablesSceneOperations
        {
            ...
            {
                UnloadCallCount++;
                return Addressables.ResourceManager.CreateCompletedOperation(default(SceneInstance), string.Empty);
            }
        }
    }
}
